...
import { displayNameValidator, emailValidator, phoneValidator } from 'src/components/validators/validators';
...
export class EditarContatoComponent implements OnInit {
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<EditarContatoComponent>,
    private contactService: ContactService
  ) {}

  ngOnInit() {
    this.formContact.patchValue(this.data)
    this.fotoUrl = this.data.foto
  }
  ...
  editContact() {
    ...
    this.data = {
      ...
      foto: base64Image.value || foto.value,
      dataCadastro: dataCadastro?.value
    };
    this.contactService.updateContact(this.data).pipe(finalize(() => this.dialogRef.close())).subscribe()
  }
  ...
  onFileChange(event: any) {
    ...
    reader.onload = () => {
      if (reader.result instanceof ArrayBuffer) {
        ...
        this.formContact.get('base64Image')?.setValue(base64ImageFinal);
